...
async function loadBeans() {
  ...
  const GOOGLE_SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vR6Zx5IKqnxZH3cyWCusoxwvAejqgVPZqnUVyo-4Xhs-kdOkFKoh0V9W5qi-12hB9BRMvOHzVlLzDNC/pub?gid=1947339755&single=true&output=csv';
  try {
...
function applyFilterAndRender() {
  ...
  if (CURRENT_TAB !== 'all') { filteredItems = filteredItems.filter(it => (it.category || '').toLowerCase() === CURRENT_TAB); }

  // 規則 3：如果搜尋框有內容，則根據關鍵字進行模糊搜尋。
  if (q) { filteredItems = filteredItems.filter(it => { const hay = [it.name_zh, it.name_en, it.origin, it.region, it.process, it.roast, it.flavor_tags_zh, it.flavor_tags_en, it.taste, ROAST_TRANSLATIONS[it.roast]].join(' ').toLowerCase(); return hay.includes(q); }); }
...
async function init() {
  ...
  // 2. 判斷是否需要顯示語言選擇畫面
  if (!LANG) { $('#langGate').style.display = 'grid'; $('#app').hidden = true; } else { $('#langGate').style.display = 'none'; $('#app').hidden = false; i18nUI(); setTab('all'); }

  // 3. 綁定所有事件監聽器（語言按鈕、頁籤、搜尋框）
  $('#langGate').addEventListener('click', (e) => { const btn = e.target.closest('[data-lang]'); if (btn) { setLanguage(btn.dataset.lang); $('#langGate').style.display = 'none'; $('#app').hidden = false; } });
  $('#langBtn').addEventListener('click', () => { $('#langGate').style.display = 'grid'; $('#app').hidden = true; });
